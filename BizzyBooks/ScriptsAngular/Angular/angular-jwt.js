...
angular.module('angular-jwt',
    ...
    ]).factory('authRetry', ['$rootScope', 'httpBuffer', function ($rootScope, httpBuffer) {
    return {
        /**
...
angular.module('angular-jwt.interceptor', [])
  .provider('jwtInterceptor', function() {
    ...
    this.$get = ["$q", "$injector", "$rootScope", "urlUtils","httpBuffer", "jwtOptions", function($q, $injector, $rootScope, urlUtils,httpBuffer, jwtOptions) {
      ...
      function isSafe (url) {
        ...
        for (var i = 0; i < options.whiteListedDomains.length; i++) {
            var domain = urlUtils.urlResolve(options.whiteListedDomains[i]).hostname.toLowerCase();
          if (domain === hostname) {
            return true;
          }
        }
...
angular.module('angular-jwt.options', [])
   ...
   */
angular.module('angular-jwt-buffer', [])
.factory('httpBuffer', ['$injector', function($injector) {
    ...
    function retryHttpRequest(config, deferred) {
        ...
        //removed token so that new token can be applied.
