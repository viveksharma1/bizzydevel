...
myApp.controller('inventorystockCntrl', ['$scope', '$http', '$timeout', '$rootScope', '$state', 'config', 'fileUpload', '$filter', function ($scope, $http, $timeout, $rootScope, $state, config, fileUpload, $filter) {
    ...
    var url = config.api + "Inventories?filter[where][visible]=true";

    $http.get(url).then(function (response) {
        $scope.InventoryList5= response.data;
    ...
    $scope.GRNDetail = function (data) {
        ...
        $scope.itemAmount1 = data.AMOUNTINR.toFixed(2)
        //vivek
        ...
        $http.get(config.api + "transactions" + "?[filter][where][no]=" + $scope.billNo).then(function (response) {
            ...
            $scope.totalCustom = Number(data.dutyAmount) / Number(data.NETWEIGHT);

        });
        $http.get(config.api + "transactions" + "?[filter][where][refNo]]=" + $scope.billNo).then(function (response) {
